...
- **Shared auth.** The one thing that must not diverge per agent is the OAuth
  credential (`.credentials.json`, `auth.json`): a rotated token in one copy would
  strand the others. It is never copied. The private copy holds a symlink to the
  template's file, and the scope binds that single file back at its template path
  — the only piece of your home an agent pane still sees.
...
| codex   | `config.toml`, `AGENTS.md`, `prompts/`, `skills/`, `rules/` | `sessions/`, `history.jsonl`, `log/`, `memories/` | `auth.json` |
...
| `shared-detached` | the auth file in the copy is no longer a symlink (the harness rewrote it in place) | `reset` re-links it to yours |
...
own configuration to own, reduces what the scope mounts from your home to one
file, and turns configuration change into an explicit, reviewable event.

The trade-off is intentional: nothing an agent configures reaches you unless you
promote it, and nothing you change reaches an existing agent unless you reset
its copy (new agents are seeded from the current template). The agent guide
...
| `panespec.configBinds` | binds only the shared auth file for the agent pane |
