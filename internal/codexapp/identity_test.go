...
func TestAttachArgvShape(t *testing.T) {
	...
	want := []string{"codex", "-c", `tui.alternate_screen="always"`, "-c", `approval_policy="on-request"`, "-c", `approvals_reviewer="auto_review"`, "--remote", "unix:///run/user/1000/codexapp/s.sock", "resume", "thr-1"}
	if strings.Join(argv, " ") != strings.Join(want, " ") {
		t.Fatalf("attach argv = %v, want %v", argv, want)
	}
	// No thread id → no resume subcommand.
	argv = AttachArgv("", "ws://127.0.0.1:4500", "")
	if strings.Join(argv, " ") != `codex -c tui.alternate_screen="always" -c approval_policy="on-request" -c approvals_reviewer="auto_review" --remote ws://127.0.0.1:4500` {
